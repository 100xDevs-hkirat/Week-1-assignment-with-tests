...
class Calculator {
  ...
  divide   = (num) => {
    if(num == 0) {
      throw Error("zero can not be divided");
    }
...
const convertToPostfix = (tokens) => {
  ...
  tokens.forEach((token) => {
    if (token === ")") {
//      console.log("operators before pop:" + operators );
      let top = operators.pop();
//      console.log("operators after pop:" + top );
      while (top !== "(") {
  ...
  console.log("postfix 1" + postFix);
  console.log("operators 1" + operators);
...
//calc = new Calculator();
//var ans = calc.calculateOwn(`$$10 +   2 *    (   6 - (4 + 1) / 2) + 7`)
//console.log(ans);
